...
if [ "$(uname)" == "Darwin" ]; then
  echo 'on OSX 64'
  curl -L https://github.com/lightbody/browsermob-proxy/releases/download/browsermob-proxy-$LATEST_BROWSERMOB/browsermob-proxy-$LATEST_BROWSERMOB-bin.zip >> browsermob.zip
  unzip browsermob.zip
  mv browsermob-proxy-$LATEST_BROWSERMOB  qa/env/bin/browsermob-proxy
  rm browsermob.zip
