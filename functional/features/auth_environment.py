...
from qa.settings import CLIENT_ID, GOOGLE_APPLICATION_CREDENTIALS, GOOGLE_API_KEY
from qa.settings import DEFAULT_WIDTH, DEFAULT_HEIGHT, DRIVER
...
from qa.settings import HOST, HOST_URL, IAP_ON, PROXY_PASSTHROUGH
from qa.settings import QA_ENV, log, JIRA_PROJECT_ABBR
from qa.settings import SELENIUM, PAGES_DICT
...
def before_all(context):
    ...
    for i in range(1, MAX_RETRIES + 1):
        ...
        if code == 200:
            break
        logging.debug('Request to "%s" failed: Status code: "%s", retrying...', HOST_URL, code)
        time.sleep(i)
...
def before_scenario(context, scenario):
    ...
    if 'validity' in context.tags:
        ...
        if IAP_ON is False:
            ...
            context.driver = context.browser.get_driver_by_name('get_authenticated_remote_html_validator')
        else:
            context.browser = Browser(proxy=context.proxy, server=context.server, passthrough=PROXY_PASSTHROUGH)
    ...
    if context.driver is not None:
        context.wait = WebDriverWait(context.driver, 20, 0.25)
        context.driver.get(HOST_URL + '/sitemap.xml')
        if IAP_ON == False and 'local' in QA_ENV:
            context.driver.add_cookie(
                {
                    'name': 'dev_appserver_login',
                    'value': 'beyondqa@bynd.com:True:118016015712924541982'
                }
            )
        ...
        if 'no-local-storage' in context.tags:
            clear_local_storage(context.driver)
        else:
            dismiss_cookie_consent(context.driver)
        if 'mobile' in context.tags:
            context.driver.set_window_size(MOBILE_WIDTH, MOBILE_HEIGHT)
        if 'custom-solution' in context.tags:
            context.driver.set_window_size(700, TABLET_HEIGHT)
        if 'tablet' in context.tags:
            context.driver.set_window_size(TABLET_WIDTH, TABLET_HEIGHT)
        if 'custom-tiles' in context.tags:
            context.driver.set_window_size(768, TABLET_HEIGHT)
...
def after_scenario(context, scenario):
    if 'browser' in context.tags or 'validity' in context.tags:
        if ('skip' not in context.tags and 'no-local' not in context.tags):
            if is_not_chromedriver() is True and 'chrome-only' in context.tags:
                return
            elif 'local-only' in context.tags and QA_ENV != 'local':
                return
            else:
                context.driver.quit()
        # if 'authenticated_safari' in DRIVER:
