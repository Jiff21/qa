...
def step_impl(context):
    assert '1' in context.current_header, 'Did not find 1, \
        instead %s' % context.current_header

@step('the CSP should contain "{csp_item}"')
def step_impl(context, csp_item):
    assert csp_item in context.current_header, 'Did not find %s, \
        instead %s' % (csp_item, context.current_header)
    ...
    for cookie in context.cookies:
        if cookie['domain'][1:] in BASE_URL:
            if cookie['name'] == '_gid' or '_gat' in cookie['name'] \
                ...
                assert cookie['secure'] == True, '%s cookie is not marked \
                    secure.' % (
                        cookie['name']
                    )
...
def step_impl(context, expected_status):
    assert context.response.status_code == expected_status, 'Expected a %d for \
        %s.' % (
            expected_status,
            context.current_url,
        )
